# 这个文件主要是用来测一下,当系统整体的数据量发生变化，会有什么样的性能变动--当然会有点不太客观，但是也算是一种参考标准。
import os
import json
import argparse
...
def Argparse():
    ...
    parser.add_argument("-task", default='SENT_CLF', type=str,
                        choices=['SENT_CLF', 'IMG_CLF', "Tabular_CLF"])  # 句子分类 or 图像分类 or 表格数据分类
    ...
    parser.add_argument("-system_data_count", default=10e10)
...
def main(dataset_name, algorithm, hypothesis, classifier_type, device, param_dict):
    ...
    # learning_rate_list = [1e-5, 5e-5, 1e-4, 5e-4]
    # learning_rate_list = [1e-6, 5e-6]
    learning_rate_list = [5e-5]

    if 'moji' in dataset_name_list[0]:
        system_data_count_list = [4000, 16000, 64000, 256000, 1024000, 1613790] # Moji数据集一共就是1613790左右
    elif 'bios' in dataset_name_list[0]:
        system_data_count_list = [1000, 4000, 16000, 64000, 257478] # Bios数据集一共就是257478左右
    else:
        system_data_count_list = [1000, 2000, 4000, 8000, 16000, 32000, 64000]
    ...
    total_Experiment_NO = len(FL_drop_rate_list) * len(epoch_T_communication_I_list) * len(split_strategy_list) * len(
        fraction_list) * len(num_clients_K_list) * len(system_data_count_list)

    # Main Loop
    for system_data_count in system_data_count_list:
                    ...
                    for algorithm_epoch_T, communication_round_I in epoch_T_communication_I_list:
                        for fraction in fraction_list:
                            for num_clients_K in num_clients_K_list:
                                ...
                                param_dict['system_data_count'] = system_data_count
                                ################################################################################################
                                # Create the log
                                log_path = os.path.join("log_path/data_scalability", param_dict['dataset_name'],
                                ...
                                result_path = os.path.join("./result_path/data_scalability", param_dict['dataset_name'],
                                                        param_dict['split_strategy'],
                                                        param_dict['algorithm'],
                                                        param_dict['hypothesis'],
                                                        str(num_clients_K) + "Clients")
                                ...
                                model_path = os.path.join("./save_path/data_scalability", param_dict['dataset_name'],
                                                          param_dict['split_strategy'],
                                                          param_dict['algorithm'],
                                                          param_dict['hypothesis'],
                                                          str(num_clients_K) + "Clients")
