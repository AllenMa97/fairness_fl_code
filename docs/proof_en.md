# PDFFed Theoretical Analysis: Prototype-Driven Fair Federated Learning

## 0. Macro-Level Proof Chain

PDFFed addresses three core challenges in fair federated learning, each supported by a dedicated proof chain.

### Overview of the Three Challenges

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        PDFFed: Three-Challenge Proof Architecture            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Challenge 1                Challenge 2                Challenge 3           │
│  (Privacy)                  (Representation Bias)      (Global Fairness)     │
│                                                                             │
│  ┌──────────────┐          ┌──────────────┐          ┌──────────────┐       │
│  │ Theorem 6    │          │ Theorem 1    │          │ Theorem 4    │       │
│  │ Converse:    │          │ Upper Bound  │          │ Approximation│       │
│  │ g → no DP    │          │ EO ≤ C·Δ_rep │          │ Bound (a)    │       │
│  └──────┬───────┘          └──────┬───────┘          └──────┬───────┘       │
│         │                         │                         │               │
│         ▼                         ▼                         ▼               │
│  ┌──────────────┐          ┌──────────────┐          ┌──────────────┐       │
│  │ Theorem 5    │          │ Theorem 2    │          │ Gradient     │       │
│  │ Positive:    │          │ Lower Bound  │          │ Alignment    │       │
│  │ Proto+LDP→DP │          │ EO ≥ 2δ-...  │          │ cos ≥ 1-O(.) │       │
│  └──────────────┘          └──────┬───────┘          └──────┬───────┘       │
│                                   │                         │               │
│                                   ▼                         ▼               │
│                            ┌──────────────┐          ┌──────────────┐       │
│                            │ Theorem 3    │          │ Descent      │       │
│                            │ Intervention │          │ Transfer (c) │       │
│                            │ CL → Δ_rep↓  │          │ δ > 0        │       │
│                            └──────────────┘          └──────────────┘       │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
### Challenge 1: Privacy Leakage Risk (Theorems 5 & 6)

```
Reverse direction (Theorem 6):
  Transmitting sensitive attributes g
    → Membership inference attack succeeds with probability 1
    → Differential privacy is broken
    ⇒ MUST NOT transmit g

Positive direction (Theorem 5):
  Transmit prototypes μ_{g,l} with ε-LDP noise
    → Distributed DP holds via composition
    → Aggregated prototypes satisfy (ε, δ)-DP
    ⇒ Privacy-preserving fairness collaboration is feasible
```

**Key insight:** Theorem 6 establishes necessity (converse proof), Theorem 5 establishes sufficiency (constructive proof). Together they justify PDFFed's design choice: transmit noisy prototypes instead of raw sensitive attributes.

### Challenge 2: Fairness Degradation from Representation Bias (Theorems 1–3)
...
Diagnose ──────────────────────────────────────────────────────────────────
    ...
    │     Large confidence gap → EO is large → fairness problem detected
    │     【Diagnostic signal: when to act】
    │
    ▼
Optimize ──────────────────────────────────────────────────────────────────
    │
    ...
    │     Reducing Δ_rep → worst-case EO is small
    │     【Optimization target: what to minimize】
    │
    ▼
Intervene ─────────────────────────────────────────────────────────────────
    │
    ├── Theorem 3 (Convergence & Δ_rep Contraction):
    │     (a) L^{(t+1)} - L^{(t)} ≤ -η ‖∇L^{(t)}‖² + ...  (convergence)
    │     (b) Δ_rep^{(t+T)} ≤ Δ_rep^{(t)} - η·λ·T·γ   (contraction)
    │     CL always runs → Δ_rep shrinks continuously, convergence preserved
    │     【Intervention mechanism: how to reduce bias】
    │
    ▼
Complete chain: Lower bound (Thm 2) → Upper bound (Thm 1) → Intervention (Thm 3)
```

**Key insight:** The three theorems form a complete feedback loop: Theorem 2 diagnoses fairness problems, Theorem 1 defines the optimization target, and Theorem 3 provides the intervention mechanism. Together they address representation-level fairness from detection through mitigation.

### Challenge 3: Global Fairness under Statistical Heterogeneity (Theorem 4)

```
Core question: Can global prototypes serve as an effective proxy for global EO?

    ├── Property (a): Approximation Bound
    │     |EO_proto - EO_global| ≤ (‖w‖/(√(2π)σ_z))·Δ_rep^proto + O(...)
    ...
    │     ⇒ Optimizing proxy aligns with optimizing true objective
    │
    ├── Property (c): Single-Step Descent Transfer
    │     EO_global^{(t+1)} ≤ EO_global^{(t)} - αλ_eo·‖∇_ψ EO_proto‖²·δ
    │     ⇒ Fairness improvement from proxy transfers to true EO
    │
    ▼
Server-side post-training with global prototypes:
  1. Aggregate prototypes → global μ_{g,l}
  2. Compute proxy: EO_proto = Σ_l |σ(w^T μ_{g0,l} + b) - σ(w^T μ_{g1,l} + b)|
  3. Fine-tune classifier head ψ = (w, b) using EO_proto
  ⇒ Improves global fairness without accessing raw local data
```

**Key insight:** Theorem 4's three properties collectively justify server-side post-training: accuracy (a), alignment (b), and transfer (c). This enables global fairness optimization using only aggregated prototype information.

### Theorem-to-Challenge Mapping

| Theorem | Type | Core Relation | Challenge |
|---------|------|---------------|-----------|
| Theorem 5 | Positive construction | Prototype aggregation + LDP satisfies DP | Challenge 1 (Privacy) |
| Theorem 6 | Converse proof | Transmitting g violates DP | Challenge 1 (Privacy) |
| Theorem 1 | Upper bound (adapted) | EO ≤ C₁ · Δ_rep + O(Δ_Σ) | Challenge 2 (Representation Bias) |
| Theorem 2 | Lower bound (original) | EO ≥ 2δ_conf - C₂ | Challenge 2 (Representation Bias) |
| Theorem 3 | Intervention (original) | CL contracts Δ_rep, convergence preserved | Challenge 2 (Representation Bias) |
| Theorem 4 | Calibration (original) | Prototype-level EO proxy ≈ true global EO | Challenge 3 (Global Fairness) |

**Supplementary note on upper bound vs. lower bound:**

- **Theorem 1 (upper bound):** States that if Δ_rep is small, then EO **cannot be large** — a sufficient condition. This guides **optimization target design**: by minimizing Δ_rep, we guarantee worst-case EO remains bounded. Formally: Δ_rep → 0 ⇒ EO ≤ ε (controllable).

- **Theorem 2 (lower bound):** States that if the confidence gap δ_conf is large, then EO **cannot be small** — a necessary condition. This serves as a **diagnostic signal**: a large confidence gap reveals that fairness is already compromised, regardless of other metrics. Formally: δ_conf large ⇒ EO ≥ δ (> 0, unavoidable).

- **Complementary roles:** Theorem 1 is prescriptive (what to optimize), Theorem 2 is diagnostic (when to act). Together they provide both the **target** and the **trigger** for the contrastive learning mechanism in Theorem 3, forming a complete feedback loop for representation-level fairness improvement.
...
| $X$ | Input space |
| $Y = \{0, 1\}$ | Label space |
| $G = \{0, 1\}$ | Sensitive attribute (group) space |
| $\mathcal{D}$ | Global data distribution |
| $\mathcal{D}_k$ | Local data distribution of client $k$ |
| $f_\phi: X \to \mathbb{R}^d$ | Feature extractor (encoder) |
| $h_\psi: \mathbb{R}^d \to \mathbb{R}$ | Classifier (linear: $h_\psi(z) = w^T z + b$) |
| $z = f_\phi(x) \in \mathbb{R}^d$ | Feature representation of sample $x$ |
| $\hat{p} = \sigma_{\text{s}}(w^T z + b)$ | Predicted probability, $\sigma_{\text{s}}$ is the sigmoid function |
| $\hat{y} = \mathbb{1}[\hat{p} \geq 0.5]$ | Predicted label |
| $\mu_{g,l} = \mathbb{E}[z \mid g, y=l]$ | Feature mean (prototype) for group $g$, label $l$ |
| $\bar{g}_{g,l}$ | Gradient prototype for group $g$, label $l$ |
| $\Delta_{\text{rep}}$ | Representation bias |
| $\rho_l$ | Gradient direction consistency |
| $\text{EO}$ | Equalized Odds gap |
| $\text{DEO}$ | Disparate Equalized Odds |
| $\sigma_{\text{s}}(\cdot)$ | Sigmoid activation function, $\sigma_{\text{s}}(t) = \frac{1}{1+e^{-t}}$ |
| $\sigma_z$ | Feature standard deviation along classification direction, $\sigma_z = \sqrt{\mathbb{E}\left[(w^T z - \mathbb{E}[w^T z])^2\right]}$ |
| $\delta_{\text{conf}}$ | Group confidence gap lower bound, $\delta_{\text{conf}} = \min_l \left| \mathbb{E}[c \mid g=0, y=l] - \mathbb{E}[c \mid g=1, y=l] \right|$ |
| $\Sigma_{g,l}$ | Feature covariance matrix for group $g$, label $l$ |
| $\Delta_\Sigma$ | Covariance difference, $\Delta_\Sigma = \max_{l} \|\Sigma_{g0,l} - \Sigma_{g1,l}\|_{\text{op}}$ |
| $K$ | Number of clients |
| $w_k = n_k / N$ | Aggregation weight of client $k$ |
| $\eta$ | Learning rate |
...
EO = Σ_{l ∈ {0,1}} | E[𝑝̂ | g=0, y=l] - E[𝑝̂ | g=1, y=l] |

where 𝑝̂ = σ_s(w^T z + b) is the predicted probability for the positive class, g ∈ {0, 1} is the group identifier, and y ∈ {0, 1} is the true label. EO measures the prediction probability difference between groups for the same label.

### Definition 1.2 (Disparate Equalized Odds, DEO)

DEO = | E[𝑝̂ | g=0, y=1] - E[𝑝̂ | g=1, y=1] |
...
For a sample (x, y, g) with task loss L_task(x, y) = ℓ(σ(w^T f_φ(x) + b), y), the feature gradient is ∇_z L_task = ∂ℓ/∂z ∈ ℝ^d.
...
where L_contrastive,k is the contrastive loss, always optimized alongside the task loss; λ > 0 is the hyperparameter controlling the strength of the contrastive loss.

### 2.2 Model Decomposition

Any supervised learning model can be naturally decomposed into an **encoder + classifier head**:

x →(f_φ) z ∈ ℝ^d →(h_ψ) 𝑝̂ = σ(h_ψ(z))
...
L_task(x, y) = ℓ(σ(w^T f_φ(x) + b), y) = -y log(𝑝̂) - (1-y) log(1-𝑝̂)

where 𝑝̂ = σ(w^T f_φ(x) + b) is the predicted probability for the positive class.
...
E[exp(λ^T (z - μ_{g,l})) | g, y=l] ≤ exp(λ^T Σ_{g,l} λ / 2), ∀ λ ∈ ℝ^d
...
where σ_z = √(E[(w^T z - E[w^T z])²]) is the standard deviation of features projected onto the classification direction.
...
### Theorem 2 (Confidence Gap and EO Lower Bound)

**Assumption:** There exists a constant δ_conf > 0 such that for all labels l ∈ {0,1}:
...
**Significance:** This is a **lower bound**. It tells us: the minimum value of EO is positively correlated with the group confidence gap δ_conf. To make EO smaller, the confidence gap between groups must first be reduced—this is a necessary condition (but not sufficient). Conversely, if the confidence gap is large (δ_conf is large), EO cannot be too small. Therefore, the confidence gap can serve as a diagnostic signal for fairness problems.

### Lemma 1 (Gradient Direction Consistency of Contrastive Loss)

Let ρ_l = cos⟨∇_φ L_contrastive, ∇_φ L_task⟩ denote the cosine similarity between the contrastive loss gradient and the task loss gradient with respect to the encoder parameters. For PDFFed's prototype-level contrastive loss L_contrastive = (1/2) Σ_l ‖μ_{g0,l} - μ_{g1,l}‖², under Assumptions 1–3:

- For any label l, ∇_φ L_contrastive aligns with the direction of reducing Δ_rep (inner product ≥ 0)
- ρ_l can be monitored online during training via gradient statistics, serving as an observational indicator of training dynamics

### Theorem 3 (Convergence of Contrastive Learning and Δ_rep Contraction)

Under Assumption 3, the local training loss L_k = L_task,k + λ · L_contrastive,k (CL always runs) satisfies:

**(a) Convergence:**

L^{(t+1)} - L^{(t)} ≤ -η ‖∇L^{(t)}‖² + (η² (L + λM) / 2) ‖∇L^{(t)}‖²

When η < 2/(L + λM), the loss decreases monotonically. Here L is the smoothness constant of the task loss, and M is the smoothness constant of the CL loss.
...
where γ = E[‖μ_{g0,l} - μ_{g1,l}‖] > 0, naturally satisfied when Δ_rep > 0.

**Significance:** (a) Always-on CL does not harm the convergence of the task loss — the only mild cost is that the learning rate upper bound tightens from η < 2/L to η < 2/(L + λM); (b) CL continuously shrinks representation bias, with the contraction rate proportional to the learning rate η, CL weight λ, and number of training steps T. Lemma 1 further guarantees that the CL gradient is naturally aligned with the direction of reducing Δ_rep, so CL introduces no directional conflict with the main task.

### Theorem 4 (Global Prototype as EO Calibration Proxy)

**Setup:** Server-side post-training updates only the classifier head ψ = (w, b), using the global prototype-level EO proxy objective for calibration:

EO_proto = Σ_{l ∈ {0,1}} |σ_s(w^T μ_{g0,l} + b) - σ_s(w^T μ_{g1,l} + b)|
...
**Significance:** This theorem answers the question "Why is using global prototypes for EO calibration at the Server effective?" — this is the core of **Challenge 3** (difficulty in guaranteeing global fairness under statistical heterogeneity). (a) shows the prototype-level EO proxy is a good approximation of the true global EO; (b) shows the optimization direction of the proxy objective is aligned with the true objective; (c) shows the descent of the proxy objective transfers to the descent of the true objective. Together, they prove that server-side post-training not only preserves model performance but also effectively improves global fairness. Key insight: the classical approximation σ_s(x) ≈ Φ(c·x) (c ≈ 2.40) bridges sigmoid and normal CDF.

### Theorem 5 (Differential Privacy Guarantee for Prototype Aggregation)

**Setup:** Client k uploads local prototypes μ_{g,l}^{(k)} in each round, and the Server performs weighted aggregation:

μ_{g,l}^{global} = Σ_{k=1}^K w_k · μ_{g,l}^{(k)}

**Assumption 4 (LDP Noise):** Each client adds ε-LDP noise N(0, σ_noise² · I_d) before uploading, where σ_noise² ≥ (2d ln(2/δ))/ε² (satisfying (ε, δ)-DP).

Under Assumption 4, the prototype aggregation process satisfies **Distributed Differential Privacy**:

Pr[A(μ_{g,l}^{global}) = t] ≤ e^ε · Pr[A(μ_{g,l}^{global,-i}) = t] + δ

where μ_{g,l}^{global,-i} is the aggregated prototype with the i-th sample removed, and A is any adversary algorithm.

**Significance:** This theorem addresses **Challenge 1** (privacy leakage risk) — proving that the prototype information transmitted in PDFFed satisfies differential privacy guarantees after adding LDP noise. Even if an adversary accesses the aggregated global prototypes, they cannot infer sensitive information about individual clients or samples.

### Theorem 6 (Privacy Risk of Transmitting Sensitive Attributes)

**Setup:** Assume a method transmits sensitive attributes g (e.g., group labels) or intermediate results containing sensitive attribute information during communication.

**Conclusion:** Any method that transmits raw sensitive attributes g **does not satisfy differential privacy**, because there exists an adversary that can perform a membership inference attack:

Pr[A(g_i) = 1 | i ∈ S] - Pr[A(g_i) = 1 | i ∉ S] = 1

The adversary can perfectly determine whether a sample belongs to a group, thus breaking privacy.

**Significance:** This theorem proves the necessity of **Challenge 1** from the converse perspective — if sensitive attributes are transmitted, privacy is directly compromised. PDFFed chooses to transmit prototypes instead of sensitive attributes precisely to avoid this risk.
...
EO_l = |E[𝑝̂ | g=0, y=l] - E[𝑝̂ | g=1, y=l]|
...
Confidence c = sigm(|z|) where z = w^T x + b is the logit. By the inverse sigmoid:
...
**Setup:** Local training loss for client k: L_k = L_task,k + λ · L_contrastive,k, where CL always runs.
...
By Assumption 3, L is (L + λM)-smooth, where L is the smoothness constant of the task loss and M is the smoothness constant of the CL loss.

By the definition of L-smoothness:

L^{(t+1)} ≤ L^{(t)} + ∇L^{(t)} · (-η ∇L^{(t)}) + ((L + λM) η² / 2) ‖∇L^{(t)}‖²
...
**Step 7: Inner product of the two gradients (Proof of Lemma 1).**
...
**Step 8: Gradient descent update (CL always runs).** The update is:

φ^{(t+1)} = φ^{(t)} - η · ∇_φ L_task - η λ · ∇_φ L_contrastive

where the second term η λ · ∇_φ L_contrastive is the additional CL update to the encoder.

**Step 9: Quantitative contraction.** By Taylor expansion:

Δ_rep^{(t+1)} = Δ_rep^{(t)} - η λ · ∇_φ Δ_rep · ∇_φ L_contrastive + O(η²)

By Step 7, ∇_φ Δ_rep · ∇_φ L_contrastive ≥ Σ_l ‖μ_{g0,l} - μ_{g1,l}‖² > 0 (strictly positive when Δ_rep > 0), so CL contributes a negative term that reduces Δ_rep.
...
**Setup:** Client k computes local prototype μ_{g,l}^{(k)} = (1/n_{g,l}^{(k)}) Σ_{i=1}^{n_{g,l}^{(k)}} z_i, adds noise, and uploads μ̃_{g,l}^{(k)} = μ_{g,l}^{(k)} + ξ_k, where ξ_k ~ N(0, σ_noise² · I_d).
...
where μ̃_{g,l}^{(k),-i} is the noisy prototype with the i-th sample removed.
...
**Setup:** Assume a method transmits raw sensitive attributes g_i.

**Step 1: Membership Inference Attack**

The adversary can design an algorithm A(g_i) to determine whether sample i belongs to the training set S:
...
**Step 3: Violation of Differential Privacy**

Differential privacy requires that for any adjacent datasets D and D':

Pr[A(M(D)) = t] ≤ e^ε · Pr[A(M(D')) = t] + δ

For mechanisms that transmit sensitive attributes, when D and D' differ only on sample i, the adversary can perfectly distinguish between them, thus violating differential privacy.

**QED.**

---

## 6. Proof Chain (Cross-Reference)

The complete proof chain with detailed challenge mappings, ASCII-art chain diagrams, and the theorem-to-challenge correspondence table is presented in **Section 0 (Macro-Level Proof Chain)**. This section retains only the core upper/lower bound note for quick reference.

**Key note on upper vs. lower bounds:**
- Theorem 1 provides an **upper bound** (reducing Δ_rep guarantees EO is small — sufficient condition)
- Theorem 2 provides a **lower bound** (confidence gap implies EO is large — diagnostic signal)
- They are complementary: Theorem 1 for **optimization target design**, Theorem 2 for **problem diagnosis**
...
$$
\mathrm{DP} \leq \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \Delta_{\text{rep}} + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right) + \Delta_{\text{label}}
$$

where

$$
\Delta_{\text{label}} = \frac{1}{2} \sum_{l} \left| \mathbb{P}(y=l \mid g=0) - \mathbb{P}(y=l \mid g=1) \right|
$$
...
$$
\mathrm{EOpp} \leq \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \Delta_{\text{rep},1} + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right)
$$

where $\Delta_{\text{rep},1} = \| \mu_{g0,1} - \mu_{g1,1} \|$ is the representation bias for positive class.
...
$$
\mathrm{DEO} = w_1 \cdot \left| \mathrm{TPR}_0 - \mathrm{TPR}_1 \right| + w_0 \cdot \left| \mathrm{FPR}_0 - \mathrm{FPR}_1 \right|
$$
...
$$
\left| \mathrm{TPR}_0 - \mathrm{TPR}_1 \right| \leq \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \| \mu_{g0,1} - \mu_{g1,1} \| + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right)
$$

For negative class $l=0$ (FPR), similarly:

$$
\left| \mathrm{FPR}_0 - \mathrm{FPR}_1 \right| \leq \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \| \mu_{g0,0} - \mu_{g1,0} \| + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right)
$$
...
$$
\begin{aligned}
\mathrm{DEO} &\leq w_1 \cdot \left[ \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \| \mu_{g0,1} - \mu_{g1,1} \| + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right) \right] \\
&\quad + w_0 \cdot \left[ \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \| \mu_{g0,0} - \mu_{g1,0} \| + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right) \right] \\[4pt]
&= \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \Bigl[ w_1 \cdot \| \mu_{g0,1} - \mu_{g1,1} \| + w_0 \cdot \| \mu_{g0,0} - \mu_{g1,0} \| \Bigr] \\
...
Let $\Delta_{\text{rep}}^{w} = w_1 \cdot \| \mu_{g0,1} - \mu_{g1,1} \| + w_0 \cdot \| \mu_{g0,0} - \mu_{g1,0} \|$, then:

$$
\mathrm{DEO} \leq \frac{\|w\|}{\sqrt{2\pi}\,\sigma_z} \cdot \Delta_{\text{rep}}^{w} + O\!\left(\frac{\|w\|^2 \Delta_\Sigma}{\sigma_z^3}\right) \cdot (w_1 + w_0)
$$
...
3. **Theorem 4's calibration proxy method applies to all metrics**—simply replace $\mathrm{EO}_{\text{proto}}$ with the prototype-level proxy of the corresponding metric
...
8. Zhong, G., Yuan, X., Shen, Y., Wang, X., & Wang, Y. (2025). Fair-FLIP: Fairness-Aware Federated Learning with Feature-Level Input Perturbation. arXiv:2507.08912.
9. Zhang, S., Li, T., Zheng, Q., Huang, H., & Qian, C. (2023). GroupMixNorm: Group-Wise Mix Normalization for Fairer Model Training. NeurIPS Workshop 2023. arXiv:2312.11969.
10. Cui, J., Wang, H., Li, J., & Zhang, Z. (2024). Understanding and Mitigating Representation Bias in Fair Classification. arXiv:2405.01112.
