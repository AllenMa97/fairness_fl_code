...
from experiment import PDFFed_Ablation_Experiment
...
def Argparse():
    ...
    parser.add_argument("-dataset", default='LFWA+', type=str, choices=['moji', 'bios', 'CelebA','UTKFace', 'FairFace', 'LFWA+', 'ADULT', 'COMPAS', 'DRUG', 'DUTCH'])
    parser.add_argument("-task", default='IMG_CLF', type=str, choices=['SENT_CLF', 'IMG_CLF', "Tabular_CLF"]) # 句子分类 or 图像分类 or 表格数据分类
    parser.add_argument("-model_type", default='ANN', type=str, choices=['ANN', 'LogisticRegression'])

    parser.add_argument("-batch_size", default=256, type=int, help="batch size") # 图像分类3060显卡极限1024 or 句子分类256,服务器4090显卡batch_size 256
    ...
    parser.add_argument("-ablation_name", default="PDF_Fed_RepModel_DynamicTrans", type=str,)
...
def main(dataset_name, algorithm, hypothesis, classifier_type, device, param_dict, ablation_name):
    ...
    # split_strategy_list = ["Dirichlet01","Dirichlet1","Uniform"]
    # split_strategy_list = ["Dirichlet01","Dirichlet05","Uniform"]
    # split_strategy_list = ["Dirichlet05"]
    ...
    if "SENT_CLF" in param_dict["task"]:
        ...
        epoch_T_communication_I_list = [(2, 500)]  # 本地走T个epoch后进行一次通信，共走T*I个epoch，每次聚合都做性能测试
    ...
    param_dict['ablation_name'] = ablation_name
                ...
                for algorithm_epoch_T, communication_round_I in epoch_T_communication_I_list:
                    for fraction in fraction_list:
                        for num_clients_K in num_clients_K_list:
                            ...
                            param_dict['tolerance_τ'] = int(algorithm_epoch_T * communication_round_I)

                            ################################################################################################
                            # Create the log
                            log_path = os.path.join("./log_path/Ablation",
                                                    param_dict['ablation_name'],
                            ...
                            result_path = os.path.join("./result_path/Ablation",
                                                       param_dict['ablation_name'],
                                                       param_dict['dataset_name'],
                                                    param_dict['split_strategy'],
                                                    param_dict['algorithm'],
                            ...
                            model_path = os.path.join("./save_path/Ablation",
                                                      param_dict['ablation_name'],
                                                      param_dict['dataset_name'],
                                                      param_dict['split_strategy'],
                                                      param_dict['algorithm'],
                            ...
                            PDFFed_Ablation_Experiment(param_dict)
                            torch.cuda.empty_cache()
...
if __name__ == '__main__':
    ...
    main(dataset_name=param_dict['dataset'],
         ...
         ablation_name=param_dict['ablation_name'],
         )


    #
    # ablation_name_list = [
    #     'PDF_Fed_Prox_Client_Sampling',
    #
    #     'PDF_Fed_Abl_local_proto_2_local_clf_loss',
    #     'PDF_Fed_Abl_local_proto_2_global_clf_loss',
    #     'PDF_Fed_Abl_global_proto_2_local_clf_loss',
    #
    #     'PDF_Fed_Abl_proto_clf_loss',
    #
    #     'PDF_Fed_Abl_pred_distribution_gap',
    #
    #     'PDF_Fed_Abl_cov_abs',
    #     'PDF_Fed_Abl_post_training',
    # ]
    #
    #
    # for item in ablation_name_list:
    #     main(dataset_name=param_dict['dataset'],
    #          algorithm=param_dict['algorithm'],
    #          hypothesis="BERTCLASSIFIER",
    #          classifier_type="linear",
