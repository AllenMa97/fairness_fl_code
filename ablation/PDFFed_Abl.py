...
def PDF_Fed_RepModel_DynamicTrans(device,
                                   global_model,
                                   ...
                                   testing_dataloader,
                                   testing_dataset_len):
    logger.info("!!!!!!!!!!!!   PDF_Fed_RepModel_DynamicTrans   !!!!!!!!!!!!!!!!")
            ...
            for epoch in range(algorithm_epoch_T):
                ...
                for batch_id, batch in enumerate(client_i_dataloader):
                    ...
                    if len(global_proto_2_local_clf_label_list) != 0:
                        ...
                        global_proto_2_local_clf_label_tensors = torch.stack(global_proto_2_local_clf_label_list).to(
                            device)
                        ...
                        if torch.isnan(global_proto_2_local_clf_loss_tmp_logit).any():
                            logger.info("### The tmp_logit is nan in global_proto_2_local_clf_loss_tmp_logit ###")
                        else:
                    ...
                    if float(batch_id) % 1 == 0:
                        ...
                        logger.info(f"### Origin task loss：{loss.item()} ;\n"
                                    ...
                                    f"global_proto_2_local_clf_loss: {round(global_proto_2_local_clf_loss, 5)} ;\n"
...
def PDF_Fed_RepModel_DynamicTrans_Prox_Client_Sampling(device,
            global_model,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_RepModel_DynamicTrans_Prox_Client_Sampling   !!!!!!!!!!!!!!!!")
    counter_RepModelTrans = 0  # 记录传输了RepModel的次数
    counter_no_RepModelTrans = 0  # 记录无传输RepModel的次数
    ...
    for iter_t in range(communication_round_I):
        ...
        for id in idxs_users:
                ...
                for batch_id, batch in enumerate(client_i_dataloader):
                    ...
                    if "SENT_CLF" in param_dict["task"]:
                        try:
                            ...
                            label_0_pred_distribution_gap += torch.norm(client_i_group_1_label_0_pred_distribution_in_one_batch - client_i_group_0_label_0_pred_distribution_in_one_batch, p=2).item()
                        except Exception:
                            # 有异常则表示batch内没抽到这个group&这个label的数据
                            pass
                        ...
                        try:
                            ...
                            label_1_pred_distribution_gap += torch.norm(client_i_group_1_label_1_pred_distribution_in_one_batch - client_i_group_0_label_1_pred_distribution_in_one_batch, p=2).item()
                        except Exception:
                            # 有异常则表示batch内没抽到这个group&这个label的数据
                            pass
                    elif "IMG_CLF" in param_dict["task"]:
                    ...
                    cov = get_cov_between_sensitive_attribute_and_prototype_decision_distance(weight_list=local_proto_weight_list, z_list=local_proto_z_list, prototype_decision_distance=local_proto_2_local_clf_decision_distance_list)
                    # print(f"cov: {cov}")
                    cov_abs = abs(cov)
            ...
            time_cost, result_dict = get_client_i_Prototype(param_dict, model, device, client_i_dataloader)
            # logger.info("~~~~~~~~~~~~~ 5. (用全局模型）计算客户的 类原型 ~~~~~~~~~~~~~~~")
            __, result_dict_from_global_model = get_client_i_Prototype(param_dict, global_model, device, client_i_dataloader)
...
def PDF_Fed_RepModel_DynamicTrans_Trainable_Constrain(device,
                                   global_model,
                                   algorithm_epoch_T, num_clients_K, communication_round_I,
                                   FL_fraction, FL_drop_rate,
                                   training_dataloaders,
                                   ...
                                   testing_dataloader,
                                   testing_dataset_len):
    logger.info("!!!!!!!!!!!!   PDF_Fed_RepModel_DynamicTrans_Trainable_Constrain   !!!!!!!!!!!!!!!!")
    counter_RepModelTrans = 0 # 记录传输了RepModel的次数
    counter_no_RepModelTrans = 0 # 记录无传输RepModel的次数
...
def PDF_Fed_Effect_Of_Alpha(device, # 没写完！先别用
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Effect_Of_Alpha   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Prox_Sampling_and_Trainable_Constrain(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Prox_Sampling_and_Trainable_Constrain   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Trainable_Constrain(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Trainable_Constrain   !!!!!!!!!!!!!!!!")
        ...
        for id in idxs_users:
            ...
            for epoch in range(algorithm_epoch_T):
                ...
                for batch_id, batch in enumerate(client_i_dataloader):
                    ...
                    for index, lamda in enumerate(lamda_list):
                        loss += lambda_param * lamda * reg_list[index]
                        clone_loss += lambda_param * lamda * reg_list[index]
...
def PDF_Fed_Prox_Client_Sampling(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Prox_Client_Sampling   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_local_proto_2_local_clf_loss(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_local_proto_2_local_clf_loss   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_local_proto_2_global_clf_loss(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_local_proto_2_global_clf_loss   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_global_proto_2_local_clf_loss(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_global_proto_2_local_clf_loss   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_proto_clf_loss(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_proto_clf_loss   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_pred_distribution_gap(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_pred_distribution_gap   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_cov_abs(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_cov_abs   !!!!!!!!!!!!!!!!")
...
def PDF_Fed_Abl_post_training(device,
            global_model,
            algorithm_epoch_T, num_clients_K, communication_round_I, FL_fraction, FL_drop_rate,
            training_dataloaders,
            training_dataset,
    ...
    logger.info("!!!!!!!!!!!!   PDF_Fed_Abl_post_training   !!!!!!!!!!!!!!!!")
