...
# FedAvg+FedProx的采样方法
# + 全局-局部原型驱动的表征空间对齐（在本地减缓数据异构导致的性能退化） 1
# + 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化） 2
# + 局部原型驱动的群组表征对比（在本地增强群组公平性） 3
# + 群组感知的全局原型分类损失差距（在本地增强群组公平性） 4
# + 群组感知的本地数据分类损失差距（在本地增强群组公平性） 5
# + 基于全局-局部原型表征差异的控制的选择上传机制（降低通信量，减缓系统异构的负面影响） 6
# + 基于全局原型与随机噪声的全局分类器监督（在全局减缓数据异构导致的性能退化） 7
...
def PDF_Fed_Abl_1(device,
    ...
    # 性能：
        # + 全局-局部原型驱动的表征空间对齐（在本地减缓数据异构导致的性能退化） 1
        # + 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化） 2
        # + 基于全局原型的全局分类器监督（在全局减缓数据异构导致的性能退化） 7
    # 公平：
        # + 群组感知的全局原型分类损失差距（在本地增强群组公平性） 4
        # + 群组感知的本地数据分类损失差距（在本地增强群组公平性） 5
    # 通信：
    ...
    prototype_MB_size = torch.rand([1, 768]).numel() * 4 / (1024 ** 2)
    ...
    for iter_t in range(communication_round_I):
        ...
        accumulated_Communication_Cost += len(idxs_users) * 4 * prototype_MB_size  # 下载全局Proto所带来的通信消耗
        ...
        for id in idxs_users:
            ...
            for epoch in range(algorithm_epoch_T):
                ...
                for batch_id, batch in enumerate(client_i_dataloader):
                    ...
                    loss = torch.sum(batch_loss) / true_batch_size
                    if float(batch_id) % 10 == 0:
                        logger.info( f"origin task loss: {loss} in batch_id:{batch_id} of epoch:{epoch} in Client:{id}.")
                    ...
                    with torch.no_grad():
                        ...
                        if len(global_group_0_label_1_prototype_list) != 0:
                            ...
                            # 全局原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                            __, tmp_logit = model.only_clf_forward(g)
                            global_group_0_label_1_clf_loss = criterion(tmp_logit.to(device),
                                                                        tmp_label.to(device)).item()

                            try:
                                client_i_group_0_label_1_one_batch_proto = client_i_group_0_label_1_feature_in_one_batch.mean(
                                    dim=0)
                                ...
                                # 局部原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                                __, tmp_logit = model.only_clf_forward(l)
                                local_group_0_label_1_clf_loss = criterion(tmp_logit.to(device),
                                                                           tmp_label.to(device)).item()
                            except Exception:
                                # 有异常则表示batch内没抽到这个group&这个label的数据
                                pass
                        ...
                        if len(global_group_1_label_1_prototype_list) != 0:
                            ...
                            # 全局原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                            __, tmp_logit = model.only_clf_forward(g)
                            global_group_1_label_1_clf_loss = criterion(tmp_logit.to(device),
                                                                        tmp_label.to(device)).item()

                            try:
                                client_i_group_1_label_1_one_batch_proto = client_i_group_1_label_1_feature_in_one_batch.mean(
                                    dim=0)
                                ...
                                # 局部原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                                __, tmp_logit = model.only_clf_forward(l)
                                local_group_1_label_1_clf_loss = criterion(tmp_logit.to(device),
                                                                           tmp_label.to(device)).item()
                            except Exception:
                                # 有异常则表示batch内没抽到这个group&这个label的数据
                                pass

                        logger.info("~~~~~~~~~~~~~ 4. 计算群组之间的损失差异 ~~~~~~~~~~~~~~~")
                        # 群组感知的全局原型分类损失差距（在本地增强群组公平性）
                        global_group_loss_gap = 0

                        if global_group_0_label_0_clf_loss != 0 and global_group_1_label_0_clf_loss != 0:
                            global_label_0_clf_loss_gap = abs(
                                global_group_0_label_0_clf_loss - global_group_1_label_0_clf_loss)
                            global_group_loss_gap += global_label_0_clf_loss_gap

                        if global_group_0_label_1_clf_loss != 0 and global_group_1_label_1_clf_loss != 0:
                            global_label_1_clf_loss_gap = abs(
                                global_group_0_label_1_clf_loss - global_group_1_label_1_clf_loss)
                            global_group_loss_gap += global_label_1_clf_loss_gap

                        if float(batch_id) % 10 == 0:
                            logger.info(
                                f"global_group_loss_gap: {global_group_loss_gap} in batch_id:{batch_id} of epoch:{epoch} in Client:{id}.")
                        loss += global_group_loss_gap

                        # 群组感知的本地原型分类损失差距（在本地增强群组公平性）
                        one_batch_group_avg_loss_gap = 0
                        '''
                        ...
                        '''
                        group_flag = protecteds.gt(0.5).to(device)
                        label_flag = labels.gt(0.5).to(device)
                        negative_group_flag = (~group_flag).to(device)
                        negative_label_flag = (~label_flag).to(device)


                        one_batch_label_0_group_0_count = sum(negative_group_flag * negative_label_flag).item()
                        one_batch_label_0_group_1_count = sum(group_flag * negative_label_flag).item()
                        one_batch_label_1_group_0_count = sum(negative_group_flag * label_flag).item()
                        one_batch_label_1_group_1_count = sum(group_flag * label_flag).item()
                        # label 0
                        if (one_batch_label_0_group_0_count != 0) and (one_batch_label_0_group_1_count != 0):
                            one_batch_group_0_avg_loss = float(
                                sum(batch_loss[negative_group_flag * negative_label_flag]) / one_batch_label_0_group_0_count)
                            one_batch_group_1_avg_loss = float(
                                sum(batch_loss[group_flag * negative_label_flag]) / one_batch_label_0_group_1_count)
                            one_batch_group_avg_loss_gap += abs( one_batch_group_0_avg_loss - one_batch_group_1_avg_loss)
                        # label 1
                        if (one_batch_label_1_group_0_count != 0) and (one_batch_label_1_group_1_count != 0):
                            one_batch_group_0_avg_loss = float(
                                sum(batch_loss[negative_group_flag * label_flag]) / one_batch_label_1_group_0_count)
                            one_batch_group_1_avg_loss = float(
                                sum(batch_loss[group_flag * label_flag]) / one_batch_label_1_group_1_count)
                            one_batch_group_avg_loss_gap += abs( one_batch_group_0_avg_loss - one_batch_group_1_avg_loss)
        ...
        total_post_training_epoch = 1
        # 测过3次的，发现性能差别不大，改成1次
        for post_training_epoch in range(total_post_training_epoch):
            # 记录GPU计算开始时间
            gpu_start_time = time.time()
            # 考虑性能
            loss_list = []
            with torch.no_grad():
                for item in post_training_feature_group_label_list:
                    ...
                    post_training_loss = torch.sum(criterion(tmp_logit.to(device), tmp_label.to(device)))
                    loss_list.append(post_training_loss.item())
            loss_list_avg = sum(loss_list) / len(loss_list)
            loss_list_abs_gap_between_group = abs((loss_list[0] + loss_list[2]) - (loss_list[1] + loss_list[3]))
            logger.info(
                f"~~~~~ In post_training_epoch:{post_training_epoch}, the loss_list_avg: {loss_list_avg}, the loss_list_abs_gap_between_group：{loss_list_abs_gap_between_group} ############")
            # 考虑公平性
            item = post_training_feature_group_label_list[0]  # 随便选一个输入来构建计算图，等下把损失再重新算回去
            if "SENT_CLF" in param_dict["task"]:
                tmp_label = torch.tensor([0, 1]).float().to(device)  # 随便选一个标签
            elif "IMG_CLF" in param_dict["task"]:
                tmp_label = torch.ones(1).to(device)
            __, tmp_logit = global_model.only_clf_forward(item)
            tmp_loss = torch.sum(criterion(tmp_logit.to(device), tmp_label.to(device)))
            post_training_loss = 0. * tmp_loss + loss_list_avg + loss_list_abs_gap_between_group
            ...
            logger.info(f"^^^^^Post training Testing, Epoch: {post_training_epoch}-----")
            if "SENT_CLF" in param_dict["task"]:
                accuracy, DEO, SPD = FL_fairness_and_accuracy_test(global_model, param_dict, testing_dataloader,
                                                                   testing_dataset_len)
                logger.info(
...
def PDF_Fed_Abl_2(device,
    ...
    for iter_t in range(communication_round_I):
        ...
        for id in idxs_users:
            ...
            for epoch in range(algorithm_epoch_T):
                ...
                for batch_id, batch in enumerate(client_i_dataloader):
                    ...
                    sent_label_flag = labels.gt(0.5).float().reshape([-1, 1])
                    sent_group_flag = protecteds.gt(0.5).float().reshape([-1, 1])
                    ...
                    logger.info("~~~~~~~~~~~~~ 2. 获取batch-level-proto / 3.局部proto与全局原型的Gap ~~~~~~~~~~~~~~~")

                    # 局部的群组表征差异
                    label_0_feature_gap, label_1_feature_gap = 0, 0

                    # 局部特征与全局原型的Gap
                    (group_0_label_0_representation_gap, group_1_label_0_representation_gap) = 0, 0
                    (group_0_label_1_representation_gap, group_1_label_1_representation_gap) = 0, 0
                    ...
                    with torch.no_grad():
                        ...
                        if len(global_group_0_label_0_prototype_list) != 0:
                            ...
                            global_group_0_label_0_clf_loss = criterion(tmp_logit.to(device), tmp_label).item()
                            try:
                                group_0_label_0_features = features[client_i_group_0_label_0_flag]
                                group_0_label_0_representation_gap = sum([ torch.norm(g - feat, p=2).item() for feat in group_0_label_0_features ]) / sum(client_i_group_0_label_0_flag)
                                l = group_0_label_0_features.mean(dim=0).to(device)
                                group_0_label_0_protocol = l
                                # 局部原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                                __, tmp_logit = model.only_clf_forward(l)
                                local_group_0_label_0_clf_loss = criterion(tmp_logit.to(device), tmp_label).item()
                            except Exception:
                        ...
                        if len(global_group_1_label_0_prototype_list) != 0:
                            ...
                            global_group_1_label_0_clf_loss = criterion(tmp_logit.to(device), tmp_label).item()
                            try:
                                group_1_label_0_features = features[client_i_group_1_label_0_flag]
                                group_1_label_0_representation_gap = sum([ torch.norm(g - feat, p=2).item() for feat in group_1_label_0_features ]) / sum(client_i_group_1_label_0_flag)
                                l = group_1_label_0_features.mean(dim=0).to(device)
                                group_1_label_0_protocol = l
                                # 局部原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                                __, tmp_logit = model.only_clf_forward(l)
                                local_group_1_label_0_clf_loss = criterion(tmp_logit.to(device), tmp_label).item()
                            except Exception:
                        ...
                        if (len(global_group_0_label_0_prototype_list) != 0) and (len(global_group_1_label_0_prototype_list) != 0):
                            label_0_feature_gap = torch.norm(group_0_label_0_protocol - group_1_label_0_protocol, p=2).item()
                        ...
                        if len(global_group_0_label_1_prototype_list) != 0:
                            ...
                            try:
                                group_0_label_1_features = features[client_i_group_0_label_1_flag]
                                group_1_label_0_representation_gap = sum([torch.norm(g - feat, p=2).item() for feat in group_0_label_1_features]) / sum(client_i_group_0_label_1_flag)
                                l = group_0_label_1_features.mean(dim=0).to(device)
                                group_0_label_1_protocol = l
                                # 局部原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                                __, tmp_logit = model.only_clf_forward(l)
                                local_group_0_label_1_clf_loss = criterion(tmp_logit.to(device), tmp_label).item()
                            except Exception:
                                # 有异常则表示batch内没抽到这个group&这个label的数据
                                pass
                        if len(global_group_1_label_1_prototype_list) != 0:
                            ...
                            try:
                                group_1_label_1_features = features[client_i_group_1_label_1_flag]
                                group_1_label_1_representation_gap = sum([torch.norm(g - feat, p=2).item() for feat in group_1_label_1_features]) / sum(client_i_group_1_label_1_flag)
                                l = group_0_label_1_features.mean(dim=0).to(device)
                                group_1_label_1_protocol = l
                                # 局部原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                                __, tmp_logit = model.only_clf_forward(l)
                                local_group_1_label_1_clf_loss = criterion(tmp_logit.to(device), tmp_label).item()
                            except Exception:
                                # 有异常则表示batch内没抽到这个group&这个label的数据
                                pass

                        if (len(global_group_0_label_1_prototype_list) != 0) and (len(global_group_1_label_1_prototype_list) != 0):
                            label_1_feature_gap = torch.norm(group_0_label_1_protocol - group_1_label_1_protocol, p=2).item()

                        logger.info("~~~~~~~~~~~~~ 4. 计算群组之间的损失差异 ~~~~~~~~~~~~~~~")
                        # 群组感知的全局原型分类损失差距（在本地增强群组公平性）
                        global_group_loss_gap = 0
                        global_label_0_clf_loss_gap = 0
                        global_label_1_clf_loss_gap = 0
                        if global_group_0_label_0_clf_loss != 0 and global_group_1_label_0_clf_loss != 0:
                            global_label_0_clf_loss_gap = abs(global_group_0_label_0_clf_loss - global_group_1_label_0_clf_loss)
                            global_group_loss_gap += global_label_0_clf_loss_gap
                        ...
                        if (one_batch_label_0_group_0_count != 0) and (one_batch_label_0_group_1_count != 0):
                            one_batch_group_0_avg_loss = float(
                                sum(batch_loss[
                                        negative_group_flag * negative_label_flag]) / one_batch_label_0_group_0_count)
                            one_batch_group_1_avg_loss = float(
                                sum(batch_loss[group_flag * negative_label_flag]) / one_batch_label_0_group_1_count)
                            one_batch_group_avg_loss_gap += abs(one_batch_group_0_avg_loss - one_batch_group_1_avg_loss)
                        # label 1
                        ...
                        reg_list = [
                            ...
                            # 局部特征与全局原型的Gap
                            group_0_label_0_representation_gap, group_1_label_0_representation_gap,
                            group_0_label_1_representation_gap, group_1_label_1_representation_gap,
                            # 全局原型的训练损失 -- 全局-局部原型驱动的局部分类器监督（在本地减缓数据异构导致的性能退化）
                        ...
                        if iter_t != 0 and float(batch_id) % 10 == 0:
                            logger.info(f"Origin task loss：{loss.item()} ;\n"
                                f"global_label_0_clf_loss_gap: {global_label_0_clf_loss_gap} ; "
                                f"global_label_0_clf_loss_gap: {global_label_1_clf_loss_gap} ; "
                                f"global_label_0_clf_loss_gap: {global_group_loss_gap} ;\n"
                                
                                f"label_0_feature_gap: {label_0_feature_gap} ; "
                                f"label_1_feature_gap: {label_1_feature_gap} ; "
                                        
                                f"group_0_label_0_representation_gap: {group_0_label_0_representation_gap} ; "
                                f"group_1_label_0_representation_gap: {group_1_label_0_representation_gap} ; "
                                f"group_0_label_1_representation_gap: {group_0_label_1_representation_gap} ; "
                                f"group_1_label_1_representation_gap: {group_1_label_1_representation_gap} ;\n"
            ...
            users_gpu_seconds_list[id] += time_cost
        ...
        for index, id in enumerate(idxs_users):
            ...
            if len(flag_list) != 0:
                if flag_list[index]:
                    ...
                    tmp = 0  # 若gap小于阈值，证明变化很小，不需要传Rep部分的参数
                    logger.info(
                        f"The prototype gap of Client {id} is narrow, do not need to upload the representation model!!")
            else:
                # flag_list为空，则证明是第一轮，需要传Rep部分的参数
                tmp = 1
                accumulated_Communication_Cost += rep_model_MB_size  # 上传一个rep所带来的通信消耗
        ...
        # total_post_training_epoch = 4 # 测了一下，感觉还是单步的性能比较好
        total_post_training_epoch = 1 # 测了一下，感觉还是单步的性能比较好
        for post_training_epoch in range(total_post_training_epoch):
            # 记录GPU计算开始时间
            gpu_start_time = time.time()
            logit_list = []
            ...
            with torch.no_grad():
                for item in post_training_feature_group_label_list:
                    ...
                    loss_list.append(post_training_loss.item())
            loss_list_avg = sum(loss_list) / len(loss_list)
            decision_gap_between_label = torch.norm(logit_list[0]-logit_list[1] ,p=2).item() + torch.norm(logit_list[2]-logit_list[3] ,p=2).item()
            loss_list_abs_gap_between_group = abs((loss_list[0] + loss_list[2]) - (loss_list[1] + loss_list[3]))
            logger.info(
                f"############ In post_training_epoch:{post_training_epoch}, the loss_list_avg: {loss_list_avg}, "
                f"the decision_gap_between_label:{decision_gap_between_label}, "
                f"the loss_list_abs_gap_between_group：{loss_list_abs_gap_between_group} ############")
            item = post_training_feature_group_label_list[0]  # 随便选一个输入来构建计算图，等下把损失再重新算回去
            if "SENT_CLF" in param_dict["task"]:
                tmp_label = torch.tensor([0, 1]).float().to(device)  # 随便选一个标签
            elif "IMG_CLF" in param_dict["task"]:
                tmp_label = torch.ones(1).to(device)
            __, tmp_logit = global_model.only_clf_forward(item[0].to(device))
            tmp_loss = torch.sum(criterion(tmp_logit.to(device), tmp_label.to(device)))
            post_training_loss = 0. * tmp_loss + loss_list_avg + decision_gap_between_label + loss_list_abs_gap_between_group
            post_training_loss.backward()
            Server_side_post_training_optimizer.step()
            gpu_end_time = time.time()
            total_gpu_seconds += (gpu_end_time - gpu_start_time)
        ...
        for id in idxs_users:
                ...
                for batch_id, batch in enumerate(client_i_dataloader):
                    ...
                    with torch.no_grad():
                        ...
                        if len(global_group_0_label_1_prototype_list) != 0:
                            ...
                            try:
                                group_0_label_1_features = features[client_i_group_0_label_1_flag]
                                group_1_label_0_representation_gap = sum(
                                    [torch.norm(g - feat, p=2).item() for feat in group_0_label_1_features]) / sum(
                                    client_i_group_0_label_1_flag)
                            except Exception:
                                # 有异常则表示batch内没抽到这个group&这个label的数据
                                pass
                        if len(global_group_1_label_1_prototype_list) != 0:
                            ...
                            try:
                                group_1_label_1_features = features[client_i_group_1_label_1_flag]
                                group_1_label_1_representation_gap = sum(
                                    [torch.norm(g - feat, p=2).item() for feat in group_1_label_1_features]) / sum(
                                    client_i_group_1_label_1_flag)
                            except Exception:
                                # 有异常则表示batch内没抽到这个group&这个label的数据
                                pass
            ...
            with torch.no_grad():
                ...
                if result_dict['client_i_group_0_label_0_prototype'] is not None:
                    ...
                    # 基于全局-局部原型表征差异的控制的选择上传机制（降低通信量，减缓系统异构的负面影响）
                    if len(global_group_0_label_0_prototype_list) != 0:
                        group_0_label_0_prototype_gap_between_client_i_and_global = torch.norm(
                            client_i_group_0_label_0_prototype - global_group_0_label_0_prototype_list[-1], p=2).item()
                        tmp_list.append(group_0_label_0_prototype_gap_between_client_i_and_global)
                ...
                if result_dict['client_i_group_1_label_0_prototype'] is not None:
                    ...
                    # 基于全局-局部原型表征差异的控制的选择上传机制（降低通信量，减缓系统异构的负面影响）
                    if len(global_group_1_label_0_prototype_list) != 0:
                        group_1_label_0_prototype_gap_between_client_i_and_global = torch.norm(
                            client_i_group_1_label_0_prototype - global_group_1_label_0_prototype_list[-1], p=2).item()
                        tmp_list.append(group_1_label_0_prototype_gap_between_client_i_and_global)
                ...
                if result_dict['client_i_group_0_label_1_prototype'] is not None:
                    ...
                    # 基于全局-局部原型表征差异的控制的选择上传机制（降低通信量，减缓系统异构的负面影响）
                    if len(global_group_0_label_1_prototype_list) != 0:
                        group_0_label_1_prototype_gap_between_client_i_and_global = torch.norm(
                            client_i_group_0_label_1_prototype - global_group_0_label_1_prototype_list[-1], p=2).item()
                        tmp_list.append(group_0_label_1_prototype_gap_between_client_i_and_global)
                ...
                if result_dict['client_i_group_1_label_1_prototype'] is not None:
                    ...
                    # 基于全局-局部原型表征差异的控制的选择上传机制（降低通信量，减缓系统异构的负面影响）
                    if len(global_group_1_label_1_prototype_list) != 0:
                        group_1_label_1_prototype_gap_between_client_i_and_global = torch.norm(
                            client_i_group_1_label_1_prototype - global_group_1_label_1_prototype_list[-1], p=2).item()
                        tmp_list.append(group_1_label_1_prototype_gap_between_client_i_and_global)
...
def PDF_Fed_Abl_6(device,
    ...
    for iter_t in range(communication_round_I):
        ...
        for id in idxs_users:
            ...
            # 计算客户的 类原型
            logger.info("~~~~~~~~~~~~~ 0. 在本地运算之前，先计算一次客户的 类原型 ~~~~~~~~~~~~~~~")
            time_cost_before_local_training, result_dict_before_local_training = get_client_i_Prototype(param_dict, model, device, client_i_dataloader)
            ...
            logger.info("~~~~~~~~~~~~~ 5. 计算客户的 类原型 ~~~~~~~~~~~~~~~")
            time_cost, result_dict = get_client_i_Prototype(param_dict, model, device, client_i_dataloader)
            users_gpu_seconds_list[id] += time_cost_before_local_training
            users_gpu_seconds_list[id] += time_cost
            ...
            logger.info("~~~~~~~~~~~~~ 5.5 客户的 类原型 进行运算 ~~~~~~~~~~~~~~~")
            with torch.no_grad():
                # Label 0, Group 0
                if result_dict['client_i_group_0_label_0_prototype'] is not None:
                    # 得到客户的原型
                    client_i_group_0_label_0_prototype = result_dict['client_i_group_0_label_0_prototype'].to(device)
                    if "SENT_CLF" in param_dict["task"]:
                        # tmp_logit [1, 2]
                        __, tmp_logit = model.only_clf_forward(client_i_group_0_label_0_prototype)
                    elif "IMG_CLF" in param_dict["task"]:
                        # tmp_predict [1, 1]
                        # tmp_logit [1, 2]
                        __, tmp_predict = model.only_clf_forward(client_i_group_0_label_0_prototype)
                        tmp_logit = torch.zeros([1, 2]).to(device)
                        tmp_logit[1] = tmp_predict.item()
                        tmp_logit[0] = 1 - tmp_logit[1]
                    elif "Tabular_CLF" in param_dict["task"]:
                        # tmp_predict [1, 1]
                        # tmp_logit [1, 2]
                        if "ANN" in str(type(model)):
                            _, tmp_predict = model.only_clf_forward(client_i_group_0_label_0_prototype)
                            tmp_logit = torch.zeros([1, 2]).to(device)
                            tmp_logit[1] = tmp_predict.item()
                            tmp_logit[0] = 1 - tmp_logit[1]
                    ...
                    logit_Y_True = float(tmp_logit[0])
