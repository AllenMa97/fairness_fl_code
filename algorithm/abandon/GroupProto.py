...
def GroupProto(device,
    ...
    for iter_t in range(communication_round_I):
        ...
        average_weight = np.array([0 for _ in range(num_clients_K)])
        ...
        for id in idxs_users:
            ...
            for epoch in range(algorithm_epoch_T):
                ...
                for batch in client_i_dataloader:
                    ...
                    print(f"Time 0: {time_0_end - time_0_start}")
                    ...
                    print(f"Time 1: {time_1_end - time_1_start}")
                    ...
                    with torch.no_grad():
                        ...
                        if len(client_i_group_1_label_0_feature_list) != 0:
                            ...
                            if len(global_group_1_label_0_prototype_list) != 0:
                                label_0_feature_gap = torch.norm(
                                    (global_group_1_label_0_prototype_list[-1].cuda() - group_1_label_0_prototype.cuda()),
                                    p=2)
                    ...
                    print(f"Time 2: {time_2_end - time_2_start}")


                    lamda_list = [1, 1, 1, 1]  # FedPro思路
            ...
            # Label 0, Protected 0
            if len(client_i_group_0_label_0_feature_list) != 0:
                client_i_label_0_prototype = torch.stack(client_i_group_0_label_0_feature_list, dim=0).cpu().mean(dim=0)
            ...
            # Label 0, Protected 1
            if len(client_i_group_1_label_0_feature_list) != 0:
                client_i_label_0_prototype = torch.stack(client_i_group_1_label_0_feature_list, dim=0).cpu().mean(dim=0)
                # 由于在内层循环容易获得权重，所以先对原型做加权，方便后续操作
                global_group_1_label_0_feature_list.append(client_i_aggregation_weight * client_i_label_0_prototype)
            # Label 1, Protected 0
            if len(client_i_group_0_label_1_feature_list) != 0:
                client_i_label_1_prototype = torch.stack(client_i_group_0_label_1_feature_list, dim=0).cpu().mean(dim=0)
                # 由于在内层循环容易获得权重，所以先对原型做加权，方便后续操作
                global_group_0_label_1_feature_list.append(client_i_aggregation_weight * client_i_label_1_prototype)
            # Label 1, Protected 1
            if len(client_i_group_1_label_1_feature_list) != 0:
                client_i_label_1_prototype = torch.stack(client_i_group_1_label_1_feature_list, dim=0).cpu().mean(dim=0)
                # 由于在内层循环容易获得权重，所以先对原型做加权，方便后续操作
            ...
            print(f"Time 4: {time_4_end - time_4_start}")
