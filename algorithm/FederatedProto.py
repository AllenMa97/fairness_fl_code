    ...
    for iter_t in range(communication_round_I):
        ...
        if (iter_t + 1) != param_dict['communication_round_I']:
            if "SENT_CLF" in param_dict["task"]:
                ...
                log_test_metrics(
                    ...
                    communication_cost=(iter_t + 1) * len(idxs_users) * 2 * prototype_MB_size
                )
                log_system_metrics(step=iter_t+1, gpu_seconds=total_gpu_seconds, 
                                   communication_cost=(iter_t + 1) * len(idxs_users) * 2 * prototype_MB_size,
                                   selected_client_count=len(idxs_users), model_mb_size=prototype_MB_size)
                flush()
        ...
        if param_dict.get('checkpoint_save_freq', 1) > 0 and iter_t % param_dict.get('checkpoint_save_freq', 1) == 0:
            save_checkpoint(
                ...
                extra_state={
                    'global_label_0_prototype_list': [p.cpu().tolist() if isinstance(p, torch.Tensor) else p for p in global_label_0_prototype_list] if global_label_0_prototype_list else [],
                    'global_label_1_prototype_list': [p.cpu().tolist() if isinstance(p, torch.Tensor) else p for p in global_label_1_prototype_list] if global_label_1_prototype_list else []
                }
            )
