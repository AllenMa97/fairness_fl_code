...
def ProxProbability(device,
    ...
    for iter_t in range(communication_round_I):
        ...
        def _train_single_client_proxprobability(client_id, device, model, param_dict, training_dataloaders, algorithm_epoch_T, use_amp, scaler, basic_path, iter_t, communication_round_I, num_clients_K, criterion):
        ...
        results = parallel_executor.run_clients(idxs_users, _train_single_client_proxprobability, param_dict=param_dict, training_dataloaders=training_dataloaders, algorithm_epoch_T=algorithm_epoch_T, use_amp=use_amp, scaler=scaler, basic_path=basic_path, iter_t=iter_t, communication_round_I=communication_round_I, num_clients_K=num_clients_K, criterion=criterion)
    ...
    save_path = os.path.join(save_dir, f"global_ProxProbability.pt")
