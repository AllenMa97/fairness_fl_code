...
def NaiveMix(device,
    ...
    for iter_t in range(communication_round_I):
        ...
        def _train_single_client_naivemix(client_id, device, model, param_dict, training_dataloaders, algorithm_epoch_T, use_amp, scaler, basic_path, iter_t, communication_round_I, num_clients_K, X_bar_list, Y_bar_list, λ, criterion):
            ...
            for epoch in range(algorithm_epoch_T):
                ...
                for batch in client_i_dataloader:
                    ...
                    with autocast_context(device, use_amp):
                        ...
                        _, mixup_outputs = model.only_clf_forward(scaled_features + (λ * mashed_input))

                        optimizer.zero_grad()

                        loss = (1 - λ) * criterion(
                            mixup_outputs, labels, 'mean'
                        ) + λ * criterion(mixup_outputs, mashed_labels, 'mean')
